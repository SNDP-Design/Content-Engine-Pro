...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<"linkedin" | "x" | "threads" | "instagram" | "reddit">("linkedin");
    ...
    <div className="flex flex-col h-screen w-full bg-[#F8FAFC] text-[#0F172A] font-sans overflow-hidden">
      ...
      <header className="flex items-center justify-between px-6 py-3.5 bg-white border-b border-slate-200 shrink-0 shadow-sm z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-bold tracking-tight text-slate-800 flex items-center gap-1.5 font-display">
              CONTENT <span className="text-indigo-600 font-extrabold">ENGINE</span>
            </h1>
            <p className="text-[10px] text-slate-400 font-mono tracking-widest leading-none">STARTUP SOCIAL SUITE</p>
          </div>
          <span className="ml-3 px-2 py-0.5 bg-slate-100 text-slate-500 text-[10px] font-bold rounded uppercase tracking-wider">v2.5 Pro</span>
        </div>

        <nav className="hidden md:flex gap-6 text-xs font-semibold text-slate-500 font-display">
        ...
        <div className="flex items-center gap-3 pl-4 border-l border-slate-200">
          <div className="text-right">
            <p className="text-xs font-bold text-slate-800">Product Builder</p>
            <p className="text-[10px] text-slate-400 font-medium font-mono">sndpdesign@gmail.com</p>
          </div>
          <div className="w-8 h-8 rounded-full bg-indigo-50 border border-slate-200 flex items-center justify-center text-indigo-600 font-bold text-xs select-none shadow-sm">
            PB
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 flex overflow-hidden min-h-0">
        ...
        <aside className="w-[380px] bg-white border-r border-slate-200 flex flex-col shrink-0 min-h-0">
          ...
          <div className="flex-1 overflow-y-auto p-4.5 space-y-4">
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-3">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-1 font-display">
                  <Sparkles className="w-3.5 h-3.5 text-indigo-500" />
                  Quick Playground Tests
                </span>
                <span className="text-[9px] text-slate-400 font-medium">Click to load demo</span>
              </div>
              <div className="grid grid-cols-3 gap-1.5">
                {SAMPLE_SOURCES.map((sam, i) => (
                  <button
                    ...
                    className="px-2 py-1.5 bg-white hover:bg-indigo-50 border border-slate-200 text-slate-700 rounded text-[10.5px] font-bold text-left truncate transition shadow-xs hover:border-indigo-200"
                    title={sam.title}
                  >
                    {sam.title}
                  </button>
            ...
            <div className="space-y-2.5">
                  ...
                  <div className="flex gap-2.5">
                    <div className="relative flex-1">
                      <input
                        ...
                        className="w-full px-3 py-2 text-xs bg-slate-50 hover:bg-slate-50/50 border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500 font-medium pr-8"
                      />
                      <Globe className="absolute right-2.5 top-2.5 w-3.5 h-3.5 text-slate-400" />
                    </div>
                    <button
                      type="button"
                      onClick={handleScrape}
                      disabled={isScraping || !urlInput.trim()}
                      className="px-3 bg-slate-900 text-white rounded-lg hover:bg-slate-800 font-bold text-xs flex items-center gap-1.5 transition select-none disabled:opacity-40"
                    >
                      {isScraping ? (
              ...
              <div className="relative">
                <textarea
                  ...
                  className="w-full h-34 px-3 py-2 text-xs bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500 font-medium resize-none leading-relaxed"
                />
                <div className="absolute right-2.5 bottom-2 text-[9px] text-slate-400 font-mono font-medium">
            ...
            <div className="border border-slate-100 rounded-xl p-3.5 space-y-3 bg-slate-50/60">
              ...
              <div className="space-y-1">
                <span className="text-[10px] text-slate-500 font-semibold block">Writing Angle / Custom Hook</span>
                <div className="grid grid-cols-2 gap-1.5">
                  {HOOK_STYLES.map((style) => (
                    ...
                    >
                      <p className="text-[10.5px] truncate">{style.name}</p>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-2">
                <div>
                  <span className="text-[10px] text-slate-500 font-semibold block mb-1">Target Persona</span>
                  <select
                    value={selectedAudience}
                    onChange={(e) => setSelectedAudience(e.target.value)}
                    className="w-full px-2.5 py-2 text-xs bg-white border border-slate-200 rounded-lg font-medium text-slate-800 focus:outline-none focus:border-indigo-500"
                  >
              ...
              <div className="space-y-1.5">
                <span className="text-[10px] text-slate-500 font-semibold block">Tone Qualities (Press Enter to add)</span>
                <div className="flex flex-wrap gap-1 mb-1.5 min-h-[22px]">
                  {toneKeywords.map((tag) => (
                    <span 
                      key={tag} 
                      className="px-1.5 py-0.5 bg-slate-200 text-slate-800 text-[9px] font-bold rounded flex items-center gap-1 select-none"
                    >
                      {tag}
                      <button 
                        type="button" 
                        onClick={() => removeToneWord(tag)}
                        className="hover:text-red-600 text-slate-500 text-[10px] font-bold"
                      >
                        ×
                      </button>
                    </span>
                  ...
                  {toneKeywords.length === 0 && (
                    <span className="text-[10px] text-slate-400 italic">No custom tone set</span>
                  )}
                </div>
                <input
                  type="text"
            ...
            <button
              ...
              className="w-full py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold text-xs shadow-lg shadow-indigo-100 hover:shadow-indigo-200 disabled:shadow-none transition-all flex items-center justify-center gap-1.5 disabled:opacity-50 select-none font-display uppercase tracking-widest cursor-pointer"
            >
              {isGenerating ? (
                <>
          ...
          <div className="h-[210px] border-t border-slate-200 flex flex-col bg-slate-50">
            <div className="px-4 py-2.5 border-b border-slate-200 flex items-center justify-between bg-white shrink-0">
              <span className="text-[10px] font-extrabold text-slate-500 uppercase tracking-wider flex items-center gap-1 font-display">
            ...
            <div className="flex-1 overflow-y-auto p-2.5 space-y-1.5">
              {history.map((record) => (
                <div
                  ...
                  className={`p-2 bg-white rounded-lg border text-left cursor-pointer transition select-none flex items-start gap-2 group hover:border-indigo-300 ${
                    generatedResult?.sourceSummary === record.generated.sourceSummary
                      ? "border-indigo-400 bg-indigo-50/20"
                      : "border-slate-200"
                  }`}
                >
                  <Bookmark className={`w-3.5 h-3.5 mt-0.5 flex-shrink-0 transition ${
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start gap-1">
                      <p className="text-[10.5px] font-bold text-slate-700 truncate leading-tight">
                        {record.title}
                      </p>
                      <span className="text-[8.5px] text-slate-400 whitespace-nowrap font-mono">{record.timestamp}</span>
                    </div>
                    <div className="flex items-center gap-1.5 mt-1">
                      <span className="text-[8px] px-1 py-0.2 bg-slate-100 text-slate-500 rounded font-medium">
                        {record.style}
                      </span>
                  ...
                  <button
                    ...
                    className="p-1 text-slate-300 hover:text-red-500 rounded text-xs select-none opacity-0 group-hover:opacity-100 transition flex-shrink-0"
                    title="Delete item"
                  >
              ...
              {history.length === 0 && (
                <div className="h-full flex flex-col items-center justify-center text-center p-4">
                  <div className="w-7 h-7 rounded-full bg-slate-100 flex items-center justify-center mb-1 text-slate-400">
                    <Layers className="w-3.5 h-3.5" />
                  </div>
                  <p className="text-[9.5px] font-semibold text-slate-500">Histories empty</p>
                  <p className="text-[8.5px] text-slate-400 max-w-[200px]">Generated startup runs will appear here for one-click reload.</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col min-h-0 bg-slate-50">
          ...
          {generatedResult ? (
            <div className="flex-1 flex flex-col min-h-0">
              ...
              <div className="m-4 mb-2 p-4 bg-white border border-slate-200 rounded-xl flex items-center justify-between gap-4 shadow-xs">
                <div className="flex items-start gap-3 flex-1 min-w-0">
                  <div className="w-10 h-10 rounded-lg bg-indigo-50 border border-slate-200 flex items-center justify-center text-indigo-600 font-bold shrink-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-slate-600 font-medium select-text line-clamp-2 mt-0.5 leading-relaxed">
                ...
                <div className="border-l border-slate-200 pl-4.5 flex-shrink-0 flex items-center gap-3">
                  <div className="text-left hidden lg:block">
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider font-display">Bulk Scheduler Mode</p>
                    <p className="text-[9px] text-slate-400 font-mono">Sync all generated platforms immediately</p>
                  </div>
                  
                  {bulkPostSuccess ? (
                    <div className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-lg text-xs font-bold animate-pulse">
                    ...
                    <button
                      ...
                      className="px-3.5 py-1.5 bg-slate-900 text-white hover:bg-slate-800 text-xs font-bold rounded-lg transition-all flex items-center gap-1.5 shadow-xs cursor-pointer select-none"
                    >
                      {isBulkPostingDemo ? (
                        <>
              ...
              <div className="flex-1 p-4 pt-0 overflow-y-auto">
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                  ...
                  <VisualMockup
                    platform="threads"
                    data={generatedResult.posts.threads}
                    onRefine={(prompt) => handleRefineSnippet("threads", prompt)}
                    isRefining={isRefining}
                  />
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-6 text-center select-none max-w-2xl mx-auto">
              ...
              <div className="w-14 h-14 rounded-2xl bg-white border border-slate-200 flex items-center justify-center text-center shadow-md mb-4 select-none animate-bounce">
              ...
              <div className="grid grid-cols-3 gap-4.5 w-full mt-8 max-w-xl">
                <div className="bg-white border border-slate-200 p-3.5 rounded-xl flex flex-col items-center text-center shadow-xs">
                  <div className="w-7 h-7 rounded-full bg-indigo-50 text-indigo-600 font-bold text-xs flex items-center justify-center mb-2 font-display">
                    1
                  </div>
                  <h3 className="text-xs font-bold text-slate-800">Supply Context</h3>
                  <p className="text-[10px] text-slate-400 mt-1">Paste metrics, product roadmaps, rough thoughts, or paste a blog URL link.</p>
                </div>
                ...
                <div className="bg-white border border-slate-200 p-3.5 rounded-xl flex flex-col items-center text-center shadow-xs">
                  ...
                  <h3 className="text-xs font-bold text-slate-800">Fine-tune Angles</h3>
                  <p className="text-[10px] text-slate-400 mt-1">Choose story styles, metric-heavy outlines, or target developer audiences.</p>
                </div>
                ...
                <div className="bg-white border border-slate-200 p-3.5 rounded-xl flex flex-col items-center text-center shadow-xs">
                  ...
                  <h3 className="text-xs font-bold text-slate-800">Refine &amp; Ship</h3>
                  <p className="text-[10px] text-slate-400 mt-1">Instantly get LinkedIn, Twitter, Reddit &amp; more formatted copy to deploy!</p>
                </div>
              </div>

              <div className="mt-8 flex flex-col items-center gap-2">
                <p className="text-[10.5px] font-semibold text-slate-400">Not sure where to start? Load a sample below:</p>
                <div className="flex gap-2">
                  <button
                    onClick={() => loadQuickSample(0)}
                    className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-bold shadow-md shadow-indigo-100 transition"
                  >
      ...
      <footer className="h-9 bg-white border-t border-slate-200 px-6 flex items-center justify-between text-[10px] font-medium text-slate-400 shrink-0 select-none z-10 font-mono">
        ...
        <div className="flex gap-4 items-center">
          <span>Active Client Session: <span className="text-slate-700 font-semibold">Startup Founder</span></span>
          <span className="text-slate-200">|</span>
          <a href="#doc-help" className="text-indigo-600 hover:underline flex items-center gap-0.5">
            Docs &amp; Prompts <ExternalLink className="w-2.5 h-2.5 inline" />
          </a>
        </div>
      </footer>
